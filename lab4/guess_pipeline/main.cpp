...
namespace
{
const char *kTrainPath = "../../guess_data/rockyou.txt";
...
void ProcessLocalTasksPipeline(PriorityQueue &q, const vector<PT> &tasks,
                               long long &generated, double &hash_time)
{
    generated = 0;
    hash_time = 0;

    const size_t batch_size = static_cast<size_t>(ReadEnvInt("PCFG_PIPELINE_BATCH",
                                                            ReadEnvInt("PCFG_HASH_BATCH", 1000000)));
    const size_t max_queued_batches = static_cast<size_t>(ReadEnvInt("PCFG_PIPELINE_QUEUE", 2));

    deque<vector<string>> pending;
    mutex pending_mutex;
        ...
        while (true)
        {
            vector<string> batch;
            {
                ...
                pending_cv.wait(lock, [&]() {
                    return producer_done || !pending.empty();
                });
                if (pending.empty())
                {
                    if (producer_done)
    ...
    auto push_batch = [&](vector<string> &batch) {
        if (batch.empty())
        {
            return;
        }
        generated += static_cast<long long>(batch.size());
        {
            unique_lock<mutex> lock(pending_mutex);
            pending_cv.wait(lock, [&]() {
                return pending.size() < max_queued_batches;
            });
            pending.emplace_back();
            pending.back().swap(batch);
        }
        pending_cv.notify_one();
    };
...
void RunMPI(int rank, int size)
{
    ...
    const bool pipeline = ReadEnvBool("PCFG_PIPELINE", false);
    const int pipeline_batch = ReadEnvInt("PCFG_PIPELINE_BATCH", ReadEnvInt("PCFG_HASH_BATCH", 1000000));
    const int pipeline_queue = ReadEnvInt("PCFG_PIPELINE_QUEUE", 2);
    double time_train = 0;
    double time_hash = 0;
    double comm_time = 0;
    ...
    if (rank == 0)
    {
        ...
        cout << "MPI pipeline:" << ((pipeline && mpi_hash) ? "on" : "off")
             << " batch:" << pipeline_batch
             << " queue:" << pipeline_queue << endl;
        auto start_train = system_clock::now();
        q.m.train(kTrainPath);
        q.m.order();
        q.init();
