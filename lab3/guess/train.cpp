...
void model::train(string path)
{
    ...
    int lines = 0;
    int train_limit = 3000000;
    if (const char *env_limit = getenv("PCFG_TRAIN_LIMIT"))
    {
        train_limit = max(1, atoi(env_limit));
    }
    cout<<"Training..."<<endl;
...
void segment::order()
{
    ...
    std::sort(ordered_values.begin(), ordered_values.end(),
              ...
              {
                  int freq_a = freqs.at(values[a]);
                  int freq_b = freqs.at(values[b]);
                  if (freq_a != freq_b)
                  {
...
void model::order()
{
    ...
    std::stable_sort(ordered_pts.begin(), ordered_pts.end(), compareByPretermProb);
