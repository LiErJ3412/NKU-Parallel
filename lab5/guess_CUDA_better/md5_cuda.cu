...
__global__ void MD5PrefixValuesKernel(const Byte *prefix, int prefix_length,
                                      const Byte *values, const int *offsets, const int *lengths,
                                      bit32 *states, int count)
{
    const int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= count)
    ...
    const Byte *value_ptr = values + offsets[idx];
    const int value_length = lengths[idx];
    const int length = prefix_length + value_length;
    const unsigned long long bit_length = static_cast<unsigned long long>(length) * 8ULL;
    const int padding_bits_mod = (length * 8) % 512;
    int padding_bits = 0;
    if (padding_bits_mod > 448)
    ...
    for (int block = 0; block < block_count; ++block)
    {
        ...
        for (int i = 0; i < 64; ++i)
        {
            ...
            if (pos < length)
            {
                ...
                {
                    value = value_ptr[pos - prefix_length];
                }
            }
            else if (pos == length)
            {
                                                  ...
                                                  bit32 *states, int count)
{
    ...
    const int prefix_bytes = prefix_length > 0 ? prefix_length : 1;
    cudaError_t status = cudaMalloc(&d_prefix, static_cast<size_t>(prefix_bytes));
    if (status != cudaSuccess)
    {
        return cudaGetErrorString(status);
    }
    if (prefix_length > 0)
    {
        status = cudaMemcpy(d_prefix, prefix, static_cast<size_t>(prefix_length), cudaMemcpyHostToDevice);
        if (status != cudaSuccess)
        {
            cudaFree(d_prefix);
            return cudaGetErrorString(status);
    ...
    const int value_bytes = total_value_bytes > 0 ? total_value_bytes : 1;
    status = cudaMalloc(&d_values, static_cast<size_t>(value_bytes));
    if (status != cudaSuccess)
    {
        cudaFree(d_prefix);
        return cudaGetErrorString(status);
    ...
    {
        status = cudaMemcpy(d_values, values, static_cast<size_t>(total_value_bytes), cudaMemcpyHostToDevice);
        if (status != cudaSuccess)
        {
            cudaFree(d_prefix);
            cudaFree(d_values);
    ...
    {
        const int blocks = (count + kThreadsPerBlock - 1) / kThreadsPerBlock;
        MD5PrefixValuesKernel<<<blocks, kThreadsPerBlock>>>(d_prefix, prefix_length, d_values,
                                                            d_offsets, d_lengths, d_states, count);
        status = cudaGetLastError();
    }
    if (status == cudaSuccess)
    {
