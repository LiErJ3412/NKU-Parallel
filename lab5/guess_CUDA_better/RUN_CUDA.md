...
进阶 direct CUDA 模式会跳过 CPU 上 `prefix + value` 的字符串生成，GPU 直接对 `prefix` 和最后一个 segment value 表做组合并哈希：

```bash
PCFG_TRAIN_LIMIT=3000000 PCFG_GUESS_LIMIT=10000000 PCFG_CUDA_DIRECT=1 ./main_cuda 2>&1 | tee cuda_direct.log
```

对比 `cuda.log` 和 `cuda_direct.log`：

- 普通 CUDA：CPU 先生成完整 guesses，再整批拷贝到 GPU 哈希。
- direct CUDA：CPU 只维护 PT 优先队列，GPU 直接处理 `prefix + values[i]`，减少 CPU 字符串拼接和 host 内存占用。
...
```bash
nsys profile -t cuda,nvtx,osrt -o pcfg_cuda_direct \
  env PCFG_TRAIN_LIMIT=3000000 PCFG_GUESS_LIMIT=10000000 PCFG_CUDA_DIRECT=1 ./main_cuda
```
