...
public:
    ...
    // 配置候选口令生成阶段的并行策略
    void ConfigureGeneration(GenerateMode mode, int threads, int threshold);

    // 对优先队列的一个PT，生成所有guesses
    void Generate(PT pt);

    string BuildPrefix(const PT &pt);
    const vector<string> *LastSegmentValues(const PT &pt);

    // CUDA direct模式：GPU直接处理prefix+最后一个segment values，不在CPU生成guesses
    int PopNextAndHashCUDA(double &hash_seconds);

    // 将优先队列最前面的一个PT
    void PopNext();
    int total_guesses = 0;
    vector<string> guesses;
    GenerateMode generate_mode = GenerateMode::Pthread;
    int generate_threads = 1;
    int generate_threshold = 50000;
};
