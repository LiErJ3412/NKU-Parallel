...
void RunSerial()
{
...
#ifdef USE_CUDA_HASH
    const bool cuda_direct = ReadEnvBool("PCFG_CUDA_DIRECT", false);
#else
    const bool cuda_direct = false;
#endif
    ...
    if (cuda_direct)
    {
        cout << "CUDA direct mode:on" << endl;
        long long total_generated = 0;
        auto start = system_clock::now();
        while (!q.priority.empty() && total_generated < generate_limit)
        {
            const int generated = q.PopNextAndHashCUDA(time_hash);
            total_generated += generated;
            if (total_generated / 100000 > (total_generated - generated) / 100000)
            {
